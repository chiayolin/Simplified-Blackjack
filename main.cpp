...
int main() {
  ...
  cout << "The aim of the game is to get a hand value of 21 or as closest as "
          "possible without exceeding it."
       << endl;
  cout << "J, Q, K, are worth 10, while the numbers are worth their own number "
          "(e.g. 10 = 10) "
       << endl;
  ...
  cout << "3) After the player receives their cards, they may choose to raise "
          "their betting amount"
       << endl;
  cout << "4) After a player raises, the dealer must match the raise and the "
          "round commences. If the dealer is unable to match the raise, they "
          "lose the round immediately"
       << endl;
  cout << "5) If a round is matched, players can either request another card "
          "or reveal their cards"
       << endl;
  cout << "6) If a player cannot pay the minimum bet, they must go all in (use "
          "all money)"
       << endl;
  cout << "7) If the player loses money at the end of a round, they lose the "
          "game!"
       << endl;
  cout << "--------------------------------------------------------------------"
          "------------"
  ...
  if (fin.fail()) {
    ...
    cout << "You have a saved game. Do you want to continue (C) or overwrite "
            "and start a new game (N): ";
    cin >> saved;
    while (saved != 'C' && saved != 'N') { // loops until a valid answer is
                                           // given
      cout << "Answer invalid. You have a saved game. Do you want to continue "
              "(C) or overwrite and start a new game (N): ";
      cin >> saved;
      if (saved == 'C' || saved == 'N') {
        break;
      }
    ...
    if (saved == 'C') { // displays contents of the file
      displaycurrent(name);
               ...
               'N') { // creates a new file with original amount of money
      newg(name);
    }
  }
  fin.close();
  ...
  char newgame = 'Y';
  while (newgame == 'Y') {
    string *deck = new string[52]; // Initialize a dynamic deck to append things
                                   // to it and also for constant updating
    string *player_deck = new string[5]; // Initialize player deck to add to it
                                         // and for constant updating
    string *comp_deck = new string[5]; // Initialize comp (dealer's deck) to add
                                       // to deck and for constant updating
    ...
    if (stoi(moneyp) == 0) {
      cout << "Sorry, you've have no money. You lose the game. Thank you for "
              "playing!"
           << endl;
      return 0;
    }
    ...
    if (stoi(moneyc) == 0) {
      cout << "Dealer has no money. You win the game! Thank you for playing!"
           << endl;
      return 0;
    }
    fin.close();
    ...
    int p_hand_value =
        player_sum(player_deck, p_counter); // gets the value of player
    cout << endl;
    ...
    if (moneypn > 0) {
      ...
      while (raise != 'Y' && raise != 'N') {
        cout << "Answer invalid. Would you like to raise (Y/N)?: "; // failsafe
                                                                    // if other
                                                                    // character
                                                                    // added
        cin >> raise;
      ...
      if (raise == 'Y') {
        ...
        if (raisevalue <= moneypn) {
          ...
          pool += raisevalue;
          if (moneycn <= 0) {
            cout << "Dealer doesn't have enough money. You win the game. Thank "
                    "you for playing!"
                 << endl; // auto lose (very player friendly)
            return 0;
          } else if (raisevalue >= moneycn &&
                     moneycn > 0) { // if dealer can't match but has more than 0
            cout << endl;
    ...
    if (skip) {
      ...
      cout << endl;
      char wl =
          determine_w_l(p_hand_value, c_hand_value); // returns if win or lost
      if (wl == 'w') {                               // win
        moneypn += pool;
        ofstream fout;
        fout.open(name + ".txt");
